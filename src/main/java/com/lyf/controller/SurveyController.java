...
import com.lyf.utils.SystemInit;
...
public class SurveyController {
    ...
    public String upload(String id,@RequestParam("file") MultipartFile multipartFile,HttpServletRequest request){
        //System.out.println("搁这2");
        //上传的位置
    ...
    public String query_detail(@PathVariable("id") Integer id,ModelMap modelMap){
        ...
        for (Question question1 : questions) {
            for (QuestionOpt questionOpt : question1.getOptions()) {
                int num = 0;
                for (AnswerOpt opt : answerOpts) {
                    if(questionOpt.getId().intValue() == opt.getOptId().intValue()){
                        num++;
                    }
                }
